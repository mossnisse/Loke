...
with their assertion and error protocol. **M4b** makes those abstractions
...
- Unions, assertions, and optional-ok exist, so `next` can return `(T, bool)` and
  an assertion's two result shapes are already one construct.
...
| Compile-time reflection | `fields_of`, `enum_values_of`, opaque descriptors, the compile-time portion of `string_view` needed by descriptor names and tags, `field.get`/`field.pointer`, `type_of`, `typeid_of`, and static `foreach` expansion |
...
| Erased views | Runtime `typeid`, `any_view` locals and parameters with assertion and type switch, `dyn Interface` values, witness materialization, and slot dispatch |
...
| Reflection descriptors | Add the semantic `string_view` identity needed for descriptor names and tags, but keep general runtime `string_view` materialization gated until M6. `meta.Field` and `meta.Enum_Value` are compiler-owned compile-time nominal struct types; their values are ordinary `Const_Aggregate`s carrying constant `string_view` name/tag values, `type`, index, and field identity, plus a marker that forbids runtime materialization. Descriptor formation starts from the resolved nominal declaration and its `Type_Info.fields`/enum-member table, which already reflects the selected active declaration, then filters symbols by visibility at the reflection lookup package. A generic clone uses its definition's lookup package. Reflection results are therefore formed per `(type, lookup package)`, not cached by type alone. `field.get`/`field.pointer` are builtins whose result type follows the descriptor constant. | `File.active_items` contains top-level items rather than the fields of a selected type. Reusing `Const_Aggregate` avoids a third constant representation while preserving design.md's visibility rule; the lookup-package key prevents a descriptor array formed inside the declaring package from leaking inaccessible members. |
...
- Add a semantic `string_view` type identity and constant string-view values for
  descriptor names and field tags; keep ordinary runtime construction,
...
- Implement `field.get`/`field.pointer` with per-element result types, the
  visibility rules that apply today, and rejection of descriptor materialization
...
- Implement `any_view`: implicit conversion at a local or parameter destination,
  ...
  assertion in both result shapes, and the type switch over concrete types.
...
- Reject type assertions and type switches on `dyn`, with the diagnostic that
