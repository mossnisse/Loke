...
| `shared(T)` internals | The control block, the strong and weak counts, the release-decrement, and the acquire fence at the final reference are ordinary Loke source in `core:sync` over the step-4 intrinsics and M5a's lifecycle hooks. | design.md's catalogue says "library records with custom lifecycle hooks and an atomic control block", and by step 5 every one of those facilities exists. |
