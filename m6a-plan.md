...
| Runtime metadata ABI | `base:runtime` is authoritative for `Type_Kind`, `Member_Info`, `Type_Info`, and `Source_Code_Location`. `Type_Info` exposes `id`, `kind`, static `name`, `size`, `align`, scalar bit/signed facts, `element`, `key`, `count`, and a read-only `members` slice. `Member_Info` carries a static name view, declared `typeid`, byte offset, and a two-word raw enum value when applicable. `Source_Code_Location` is `{file, procedure: string_view, line, column: int}`. | The emitter and library need one frozen public layout before either side is written. Static views require no runtime ownership, and the fields cover formatting and data editors without exposing compiler-only `type` descriptors. |
...
string assignment shares atomically, copy does not, the last handle frees with
